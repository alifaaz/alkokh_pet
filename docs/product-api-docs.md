...
> 💡 **SKU is optional** — if not provided, auto-generates as `SKU-00001`, `SKU-00002`, etc.

#### Create without SKU (auto-generated)
\`\`\`json
{
...
#### Response includes
- \`qty\` — live stock from ERPNext Bin
- \`images\` — all uploaded images (with \`custom_is_default\` flag)
- \`category_image\` — image of the category (Item Group)
