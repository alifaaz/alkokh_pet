...
@frappe.whitelist()
def start_visit(case_sheet_name: str) -> dict:
	...
	if existing_visit:
		frappe.throw(
			_("Vet Visit {0} already exists for this Case Sheet with status {1}.").format(
				frappe.bold(existing_visit.name), frappe.bold(existing_visit.status)
			),
